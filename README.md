...
5. **Set Working Directory**:
   - Set the working directory to the project root (`oci-python-scripts-grok`):
...
6. **Run Scripts**:
   ...
   - Click "Run Python File" (top-right) or use the terminal:
     ```bash
     python src/oci-simple.py -c ocid1.compartment.oc1..example
     ```
     or
     ```bash
     uv run src/oci-simple.py -c ocid1.compartment.oc1..example
     ```
   - For GUI:
     ```bash
     python src/oci-tkinter-display.py -c ocid1.compartment.oc1..example -b lightblue
     ```
         ...
         "configurations": [
             {
                 ...
                 "type": "python",
                 "request": "launch",
                 "program": "${workspaceFolder}/src/oci-tkinter-display.py",
...
import oci
import logging
from utils import setup_logging, initialize_oci, connect_to_oci, validate_input, get_oci_client
...
import oci
import logging
from utils import setup_logging, get_oci_client, validate_input
...
from utils import setup_logging, get_oci_client, initialize_oci
...
3. **Test Non-OCI Environment**:
   ...
   python src/oci-parallel-search.py -v
   ```
...
4. **Test Scripts**:
   - **Using `python`**:
     - `oci-simple.py`:
       ...
       ```
       2025-06-25 16:01:00,000 - MainThread - INFO - Processing compartment: ocid1.compartment.oc1..example
     ...
     - `oci-tkinter-display.py`:
       ```bash
       python src/oci-tkinter-display.py -c ocid1.compartment.oc1..example -b lightblue -v
       ```
       Expected output: GUI with 800x600 window, `tksheet` (250-pixel columns), showing instance details (e.g., `["instance-1", "ocid1.instance.oc1..abc", "VM.Standard2.1", "Running"]`).
     - `oci-parallel-paginated.py`:
       ```bash
       python src/oci-parallel-paginated.py -m 2 -v
       ```
       Expected output (example):
       ```
       2025-06-25 16:01:00,000 - MainThread - INFO - Fetching compartments
       2025-06-25 16:01:00,001 - MainThread - INFO - Found 2 compartments
       2025-06-25 16:01:00,002 - Thread-1 (work) - INFO - Compartment ocid1.compartment.oc1..example1: 2 instances: ['instance-1', 'instance-2']
     ...
     - `oci-parallel-search.py`:
       ```bash
       python src/oci-parallel-search.py -m 2 -v
       ```
       Expected output (example):
       ```
       2025-06-25 16:01:00,000 - MainThread - INFO - Searching for running compute instances
       2025-06-25 16:01:00,001 - MainThread - INFO - Found 3 running instances
       2025-06-25 16:01:00,002 - Thread-1 (work) - INFO - Instance instance-1 in compartment MyCompartment1: Size VM.Standard2.1
       ...
       ```
   - **Using `uv run`**:
     - Replace `python` with `uv run` in the above commands (e.g., `uv run src/oci-simple.py -c ocid1.compartment.oc1..example -v`). Output remains the same.

## Step 10: Commit and Push
Stage and commit updated files:
```bash
git add src/ .pre-commit-config.yaml
git commit -m "Fix ModuleNotFoundError by simplifying directory structure and using absolute imports"
git push origin main
```

## Step 11: Running the Scripts
Run from the project root (`oci-python-scripts-grok`):
- **oci-simple.py**:
  ```bash
  python src/oci-simple.py -c ocid1.compartment.oc1..example -v
  ```
  or
  ```bash
  uv run src/oci-simple.py -c ocid1.compartment.oc1..example -v
  ```
- **oci-tkinter-display.py**:
  ```bash
  python src/oci-tkinter-display.py -c ocid1.compartment.oc1..example -b lightblue -v
...
- **Fix for ModuleNotFoundError**:
  - Moved `utils.py` to `src/` and scripts to `src/`.
  - Changed imports to `from utils import ...`.
  - Run scripts directly with `python src/<script>.py` or `uv run src/<script>.py` from the project root.
- **Script Functionality**:
  - `oci-simple.py`: Lists name, status in a compartment (`-c`, `-v`).
  - `oci-tkinter-display.py`: Shows name, OCID, size, status (Running/Starting/Stopping/Not Running) in an 800x600 GUI (`-c`, `-b`, `-v`).
  - `oci-parallel-paginated.py`: Lists names by compartment with pagination (`-m`, `-v`).
  - `oci-parallel-search.py`: Lists running instances’ names/sizes using `ResourceSearchClient` and `ComputeClient.get_instance` (`-m`, `-v`).
- **Key Features**:
  - Centralized OCI handling in `utils.py`.
...
- **Troubleshooting**:
  ...
  - In VSCode, confirm interpreter is `.venv/bin/python` and working directory is project root.
  - Run from project root (`oci-python-scripts-grok`).
  - Share output for issues:
    ```bash
    python src/oci-parallel-search.py -v
    ```

The project is now fully finalized, with no `ModuleNotFoundError` when running directly with `python` or `uv run`. All scripts are functional, and the guide is streamlined. Let me know if you need further assistance with deployment, testing, or additional features!